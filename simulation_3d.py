"""
MagnoGlove Pro – 3D Simulation Module  v3.1
=============================================
"""
...
ATTACH_DIST     = 0.95              # Slightly larger snap radius
GRAVITY         = -18.0             # Stronger gravity for weightier feel
MAGNET_FRICTION = 2.8               # Frame-rate independent air resistance
TABLE_FRICTION  = 4.5               # Friction when sliding on table
RING_COUNT      = 7
...
class MetalObject:
    ...
    def _build(self):
        ...
        if t == 'SPHERE':
            ...
            shaft_col = color.rgb(
                max(0, int(c.r * 255) - 30), max(0, int(c.g * 255) - 30), max(0, int(c.b * 255) - 30)
            )
            self._add('cylinder', shaft_col, Vec3(0.15, 0.42, 0.15), Vec3(0, -0.29, 0))
        elif t == 'NUT':
            ...
            shaft_col = color.rgb(
                max(0, int(c.r * 255) - 25), max(0, int(c.g * 255) - 25), max(0, int(c.b * 255) - 25)
            )
            self._add('cylinder', shaft_col, Vec3(0.11, 0.46, 0.11), Vec3(0, -0.28, 0))
        elif t == 'CAP':
            self._add('cylinder', c, Vec3(0.38, 0.11, 0.38))
            shaft_col = color.rgb(
                min(255, int(c.r * 255) + 20), min(255, int(c.g * 255) + 20), min(255, int(c.b * 255) + 20)
            )
            self._add('cylinder', shaft_col, Vec3(0.15, 0.38, 0.15), Vec3(0, -0.25, 0))
...
class SimController(Entity):
    ...
    # ── Object physics (ENTERPRISE REFACTOR) ──────────────────────────────────
        ...
        for idx, obj in enumerate(self.metal_objects):
            if self.magnet.is_active():
                ...
                if dist < ATTACH_DIST:
                    ...
                    # Smooth lock-in
                    obj.position = lerp(obj.position, Vec3(tx, ty, tz), min(dt * 15, 1))
                    obj.velocity = Vec3(0, 0, 0)
                    obj.attached = True
                    obj.color    = _lc(obj.color, obj.c_pulled, dt * 8)
                    obj.rotation_y += (200 if self.current_magnet == MagnetState.ON else 90) * dt

                else:
                    # PROPER PHYSICS INTEGRATION
                    accel_mag = self.magnet.get_pull_speed(dist)
                    acceleration = direction.normalized() * accel_mag
                    
                    obj.velocity += acceleration * dt
                    # Frame-rate independent friction formula: V = V * e^(-friction * dt)
                    obj.velocity *= math.exp(-MAGNET_FRICTION * dt)
                    obj.position += obj.velocity * dt
                    obj.attached  = False

                    prox = max(0.0, 1.0 - dist / 8.0) * self.magnet.strength
                    obj.color = _lc(obj.color, obj.c_pulled, dt * 5.0 * prox)

            else:
                # ── Release / Gravity Dynamics ──
                obj.attached = False

                if obj.position.y > OBJECT_Y + 0.05:
                    # Falling
                    obj.velocity.y += GRAVITY * dt      
                    obj.velocity *= math.exp(-MAGNET_FRICTION * 0.2 * dt) # Less air resistance while falling
                    obj.position += obj.velocity * dt

                    # Floor Collision
                    if obj.position.y <= OBJECT_Y:
                        obj.position.y  = OBJECT_Y
                        obj.velocity.y *= -0.35         # Bounciness
                        obj.velocity.x *= 0.50          # Floor friction bite
                        obj.velocity.z *= 0.50
                else:
                    # Resting on table
                    obj.position.y = OBJECT_Y
                    ...
                    # Drift slowly to original layout positions
                    obj.position.x = lerp(obj.position.x, obj.rest_pos.x, dt * 1.5)
                    obj.position.z = lerp(obj.position.z, obj.rest_pos.z, dt * 1.5)
                    obj.color      = _lc(obj.color, obj.c_idle, dt * 3.0)
...
class MagnoGloveSimulation:
    ...
    def __init__(self, shared_state: dict):
        ...
        self.app          = Ursina(
            title     = "MagnoGlove Pro  v3.1 – Electromagnetic Glove Simulation",
            borderless= False,
            fullscreen= False,
        )
        window.color = C_BG
    ...
    def _build_ui(self):
        ...
        # Cleaned UI text (Removed Unicode emojis/bars to prevent console warnings on macOS)
        Text(text="*** MagnoGlove Pro v3.1 ***",
